...
- Enhanced picker = Popover top layer + `@lekoala/floating` fixed-coordinate placement (`autoUpdate()` follows scroll/resize while open). Do not add `getBoundingClientRect()` placement, global scroll/resize positioning listeners, `dropdownParent`, CSS Anchor Positioning, or modal-specific positioning hacks.
