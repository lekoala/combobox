...
- Enhanced picker = Popover top layer + `@lekoala/floating` placement (`autoUpdate()` follows scroll/resize while open). The coordinate model is read once per opening via the JS-only `coordinateSpace` option (`"viewport"` default → viewport + fixed, `"document"` opt-in → document + absolute; an invalid value falls back to `"viewport"`). No layout inference is performed. A change while open applies to the next opening. Do not add `getBoundingClientRect()` placement, global scroll/resize positioning listeners, `dropdownParent`, CSS Anchor Positioning, or modal-specific positioning hacks.
...
- `guards.add` gates brand-new creations only; selecting an existing option never runs it. A rule covering every addition is a synchronous `combobox:beforeselect` handler on the native source.
