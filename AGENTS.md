...
- Enhanced picker = Popover top layer + `@lekoala/floating` placement (`autoUpdate()` follows scroll/resize while open). The coordinate model is resolved once per opening via the JS-only `coordinateSpace` option (`"auto"` default: document flow → document + absolute, fixed/sticky/modal/popover → viewport + fixed; `"document"`/`"viewport"` force one model). Do not add `getBoundingClientRect()` placement, global scroll/resize positioning listeners, `dropdownParent`, CSS Anchor Positioning, or modal-specific positioning hacks.
