/*** @lekoala/combobox v0.3.0 - https://github.com/lekoala/combobox ***/
(() => {
  ...
  function computeFilteredItems(combobox, items, query) {
    ...
    let visible = items.filter((item) => {
      if (combobox.isMultiple && item.selected && !combobox.options.toggleSelected)
        return false;
  ...
  class Combobox {
    ...
    #restorePosition({ option, index, scrollTop }) {
      if (instances.get(this.source) !== this || !this.isOpen())
        return;
      if (option) {
