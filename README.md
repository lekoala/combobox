...
Attributes map to options (`create`, `placeholder`, `search`, `min-chars`, `max-items`, `selection-order`, `separators`, `load-on-empty`, `allow-empty-option`, `debounce`). JavaScript-only behavior (remote `load`, `create`, renderers) is passed through `<element>.configure({ ... })`. See [Element API](docs/API.md#element-and-registration).
...
test/browser/smoke.spec.js starter Playwright tests
```
...
The browser suite is the important suite: Popover, focus, form validation, keyboard behavior and Anchor Positioning need a real browser. Unit tests should be reserved for pure matching/tokenization/order helpers once those helpers are extracted.
